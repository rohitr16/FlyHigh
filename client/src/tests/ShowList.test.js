...
it("renders with empty offers", () => {
    const view = render(<ShowList  offersList={offerlist} airportCodeToCityMap={airportCodeToCityMap} />);
    expect(view.container.getElementsByClassName('card__side').length).toBe(0);
    expect(view).toMatchSnapshot(); 
  });

  it("renders with  offers", () => {
    const view = render(<ShowList  offersList={data} airportCodeToCityMap={airportCodeToCityMap} />);
